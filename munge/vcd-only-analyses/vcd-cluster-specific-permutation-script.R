...
clusters <- as.list(1,3:24) # 2 doesn't have any VCDs with known serotypes
names(clusters) <- clusters
plan(multisession)

tictoc::tic()
permuted_radii <- future_map_dfr(clusters[2], ~cluster_wrap_function(
  clusters = .x,
  df = perm_vcd,
  perms = perms,
  r1 = r_lower,
...
output_cluster_permutations_vcds <- list(permuted_radii = permuted_radii,
                                permuted_concentric = permuted_concentric)

save(output_cluster_permutations_vcds, 
     file = "cluster-specific-permutation-output-vcds-only.RData")
